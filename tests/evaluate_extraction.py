...
def compare_fields(
    predicted: Dict[str, Any], golden: Dict[str, Any]
    ...
    for key in all_keys:
        ...
        if g_str == p_str:
            ...
            if predicted_val is not None:
                stats["fp"] += 1
                errors.append(
                    f"  - Field '{key}': Expected '{g_str}', but Predicted '{p_str}'"
                )
            if golden_val is not None and predicted_val is None:
                stats["fn"] += 1
                errors.append(
                    f"  - Field '{key}': Expected '{g_str}', but missed (Predicted None)"
                )
    ...
    if predicted_lines == golden_lines:
        ...
        stats["fn"] += 1
        errors.append(
            f"  - Field 'line_items': Expected {golden_lines} items, but Predicted {predicted_lines} items"
        )
