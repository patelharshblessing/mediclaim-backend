...
#         data = st.session_state.extracted_data.get("extracted_data", {})
...
if st.session_state.access_token is None:
    ...
    if uploaded_file:
        ...
        if st.button("Process Bill"):
            if os.path.exists(cache_path) and not force_reprocess:
                st.info(f"Loading data from cache: {cache_path}")
                with open(cache_path, "r") as f:
    ...
    if st.session_state.extracted_data:
        ...
        if st.button("Adjudicate Claim"):
            ...
            adjudication_payload = {
                "extracted_data": final_data_for_adjudication,
                "insurance_details": {
                    "policy_number": policy_number,
                    "insurance_provider": insurance_provider,
                },
            ...
            with st.spinner("Applying rules and adjudicating..."):
                response = requests.post(
                    f"{API_BASE_URL}/claims/adjudicate/{st.session_state.claim_id}",
            ...
            with st.expander("Show Performance & Processing Details ⏱️"):
                ...
                with c1:
                    ...
                    st.metric(
                        ...
                        value=f"{perf.get('extract_processing_time_sec', 0):.2f} sec",
                    )
                    st.metric(
                        label="Pages Processed",
                        value=f"{perf.get('num_pages', 0)}",
                    )
                ...
                with c2:
                    ...
                    st.metric(
                        label="Total Time",
                        value=f"{perf.get('adjudicate_processing_time_sec', 0):.2f} sec",
                    )
                    st.metric(
                        label="Items Processed",
                        value=f"{perf.get('total_items_processed', 0)}",
                    )
                    st.metric(
                        label="Rules Applied",
                ...
                st.text(
                    f"IRDA Filter: {perf.get('time_irda_filter_sec', 0):.3f} sec\n"
                    f"Rule Matching (AI): {perf.get('time_rule_matching_sec', 0):.3f} sec\n"
                    f"Rule Application (AI): {perf.get('time_rule_application_sec', 0):.3f} sec\n"
                    f"Sanity Check (AI): {perf.get('time_sanity_check_sec', 0):.3f} sec"
                )
