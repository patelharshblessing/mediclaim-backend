...
from typing import (
    List,
)  # CHANGE: remove unused Tuple/date imports, add List for typing clarity
...
async def adjudicate_claim(
    ...
    """
    try:
        logger.info("Starting adjudication for claim.")
        ...
        log_time("Rules Matching", time.time() - step2_start_time)
        ...
        log_time("Rules Application", time.time() - step3_start_time)
        ...
        for itm in adjudicated_claim.adjudicated_line_items:
            if (
                itm.status != "Disallowed"
                and itm.reason != "Non-payable item as per IRDAI guidelines."
            ):
        ...
        log_time("Sanity Check", time.time() - step5_start_time)

        print("--- ✅ Adjudication and Final Audit Complete ---")
        logger.info("Adjudication completed successfully.")
        return adjudicated_claim
    except Exception as e:
        logger.error("Error during claim adjudication: %s", e, exc_info=True)
        raise


# Helper function for logging time
def log_time(step: str, duration: float):
    with open("time_logging.txt", "a") as log_file:
        log_file.write(f"{step}: {duration:.2f} seconds\n")
