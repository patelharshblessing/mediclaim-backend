...
if st.session_state.access_token is None:
            ...
            if os.path.exists(cache_path) and not force_reprocess:
                ...
                with st.spinner("Calling AI to extract data... This may take a moment."):
                    ...
                    if response.status_code == 200:
                        ...
                        st.success(f"Data extracted and saved to cache! Please review below.")
                    else:
    ...
    if st.session_state.extracted_data:
        ...
        st.info(f"Fields with low confidence (< {CONFIDENCE_THRESHOLD:.0%}) are highlighted in red. Please verify and edit them as needed.")
        ...
        cols = st.columns(3)
        edited_header = {}
        header_keys = ["hospital_name", "patient_name", "bill_no", "bill_date", "admission_date", "discharge_date", "net_payable_amount"]
        ...
        for i, key in enumerate(header_keys):
            with cols[i % 3]:
                ...
                if confidence < CONFIDENCE_THRESHOLD:
                    st.markdown(f'<p style="color:red;">{label}</p>', unsafe_allow_html=True)
                else:
                    st.write(label)
            ...
            for i, item in enumerate(data['line_items']):
                ...
                with col1:
                    ...
                    if field_data.get('confidence', 1.0) < CONFIDENCE_THRESHOLD:
                        st.markdown(f'<p style="color:red;">{label_text}</p>', unsafe_allow_html=True)
                    else:
                ...
                with col2:
                    ...
                    default_qty = 1.0 if quantity_value is None else float(quantity_value)
                ...
                with col3:
                    ...
                    default_price = total_amount_value if price_value is None else float(price_value)
